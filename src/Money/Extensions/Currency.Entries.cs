...
namespace Lsquared.Extensions
{
    ...
    partial struct Currency
    {
        ...
        private static void ReadEntries()
        {
            ...
            using (var stream = typeof(Money).Assembly.GetManifestResourceStream(@"Lsquared.data.json.zip"))
#else
            using (var stream = typeof(Money).GetTypeInfo().Assembly.GetManifestResourceStream(@"Lsquared.data.json.zip"))
#endif
